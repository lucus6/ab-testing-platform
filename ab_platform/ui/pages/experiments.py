...
def show():
    ...
    with tab1:
        _show_experiment_list()

    with tab2:
        _show_create_form()


def _show_experiment_list():
    session = get_session()
    try:
        ...
        for exp in experiments:
            with st.container():
                ...
                with col5:
                    ...
                    st.caption(g_text)
                with col6:
                    _show_status_buttons(session, exp)
                st.markdown("---")
    finally:
        session.close()


def _show_status_buttons(session, exp):
    """按当前状态展示可用操作"""
    cols = st.columns(3)
    idx = 0
    if exp.status == "draft":
...
def _show_create_form():
    ...
    with st.form("create_experiment"):
        ...
        owner = st.text_input("负责人", placeholder="输入你的名字")
